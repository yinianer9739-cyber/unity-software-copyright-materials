...
1. `创建临时工程副本自动截图`：recommended and slower. Copy the Unity project to an isolated temporary working directory, excluding generated folders such as `Library`, `Temp`, `Logs`, `Build`, `obj`, and package/cache outputs. Run screenshot automation inside the copy. The user's currently opened Unity instance must not be touched, and the user may continue normal computer work.
2. `接管当前 Unity 实例自动截图`：faster but risky. Require the user to explicitly reply `我已保存 Unity 当前工作，并允许 Codex 接管当前 Unity 实例进行自动截图。` before proceeding. During this mode, the assistant may switch scenes, enter/exit Play Mode, trigger UI events, and change the current game state. The user should not operate Unity until the action completes.
...
8. Scan `<package-dir>/截图/` and produce a screenshot inventory under `<package-dir>/报告/`. Reject any image whose resolved path is outside `<package-dir>/截图/`; final generation must not copy or reuse external screenshots.
...
14. For the registration application form, apply `references/application-form-field-mapping.md`, fill the source program line count from the normalized source count, and report every unresolved mapped field.
...
- the user chose automatic screenshots but has not selected temporary-copy mode or explicitly confirmed current-instance takeover mode;
